...

{
    ...
    do {
        ...
        if (FunctionEntry != NULL) {
            ExceptionRoutine = MyRtlVirtualUnwind(UNW_FLAG_EHANDLER,
                                                ImageBase,
                                                ControlPc,
                                                FunctionEntry,
                                                &ContextRecord1,
            ...
            /*if (RtlpIsFrameInBounds(&LowLimit, EstablisherFrame, &HighLimit) == FALSE) {
                ...
                do {
                   ...
                   /* if ((NtGlobalFlag & FLG_ENABLE_EXCEPTION_LOGGING) != 0) {
                    ...
                    printf("call handler for exception\n");
                    Disposition =
                        RtlpExecuteHandlerForException(ExceptionRecord,
                                                       EstablisherFrame,
                                                       ContextRecord,
                                                       &DispatcherContext);
                    printf("call handler for exception ret %d\n", Disposition);
    ...
    } while (/*RtlpIsFrameInBounds(&LowLimit, (ULONG64)ContextRecord1.Rsp, &HighLimit) == TRUE*/ TRUE);
...

{
    ...
    do {
        ...
        if (FunctionEntry != NULL) {
            ...
            ExceptionRoutine = MyRtlVirtualUnwind(UNW_FLAG_UHANDLER,
                                                ImageBase,
                                                ControlPc,
                                                FunctionEntry,
                                                PreviousContext,
            ...
            /*if ((RtlpIsFrameInBounds(&LowLimit, EstablisherFrame, &HighLimit) == FALSE) ||
                ...
                do {
                    ...
                    DispatcherContext.HistoryTable = HistoryTable;
                    DispatcherContext.ScopeIndex = ScopeIndex;
                    printf("call handler for unwind\n");
                    Disposition =
                        RtlpExecuteHandlerForUnwind(ExceptionRecord,
                                                    EstablisherFrame,
                                                    CurrentContext,
                                                    &DispatcherContext);
                    printf("call handler for unwind ret %d\n", Disposition);
                    ...
                    case ExceptionCollidedUnwind :
                        ...
                        PreviousContext = &LocalContext;
                        RtlpCopyContext(PreviousContext, CurrentContext);
                        MyRtlVirtualUnwind(UNW_FLAG_NHANDLER,
                                         ImageBase,
                                         ControlPc,
                                         FunctionEntry,
                                         PreviousContext,
...

{
    ...
    //
    Destination->ContextFlags |= Source->ContextFlags & 0x10000F;
    ...
    Destination->FltSave.ControlWord = Source->FltSave.ControlWord;
    Destination->FltSave.StatusWord = Source->FltSave.StatusWord;
    Destination->FltSave.TagWord = Source->FltSave.TagWord;
    Destination->FltSave.ErrorOpcode = Source->FltSave.ErrorOpcode;

    Destination->Header[1] = Source->Header[1];
    Destination->Legacy[0] = Source->Legacy[0];
    Destination->Legacy[1] = Source->Legacy[1];
    Destination->Legacy[2] = Source->Legacy[2];
    Destination->Legacy[3] = Source->Legacy[3];
    Destination->Legacy[4] = Source->Legacy[4];
    Destination->Legacy[5] = Source->Legacy[5];
    Destination->Legacy[6] = Source->Legacy[6];
    Destination->Legacy[7] = Source->Legacy[7];
    ...
    const ULONG_PTR* lpArguments)
{
    ...
    ExceptionRecord.ExceptionAddress = MyRaiseException;
    if (lpArguments)
    {
        v4 = 15;
        ...
        if (v4)
            memmove(ExceptionRecord.ExceptionInformation, lpArguments, 8i64 * v4);
    }
    else
    {
        ExceptionRecord.NumberParameters = 0;
    }
    MyRtlRaiseException(&ExceptionRecord);
}